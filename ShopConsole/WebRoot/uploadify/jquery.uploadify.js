...
(function ($) {
            ...
            return this.each(function () {
                ...
                if (flashInstalled) {
                    ...
                    swfuploadify.queueData = {
                        ...
                        errorMsg: '某些文件未能成功添加到队列中：'
                    };

                    // Save references to all the objectsqueueLength
                    ...
                    $wrapper.hover(function () {
                        //$button.css({ "background-image": "url(/Scripts/uploadify/selectfile_hover.png)" });
                        //$button.attr('class', 'btn btn-primary-hover');
                        $button.attr('class', settings.buttonHoverClass);
                        //$button.toggleClass('hover');
                    }, function () {
                        //$button.css({ "background-image": "url(/Scripts/uploadify/selectfile.png)" });
                        //$button.attr('class', 'btn btn-primary');
                        $button.attr('class', settings.buttonClass);
                    });
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = '某些文件未能成功添加到队列中：';
        ...
        onSelect: function (file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    var replaceQueueItem = ShowConfirm('同名称的文件 "' + file.name + '" 已经在队列中。\n您希望替换队列中的现有项目？');
                    if (!replaceQueueItem) {
            ...
            if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
                switch (errorCode) {
                    ...
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n选定文件 "' + file.name + '" 超过了文件大小限制 (' + settings.fileSizeLimit + ')。';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n选定文件 "' + file.name + '" 为空。';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n选定文件 "' + file.name + '" 的文件类型不被允许 (' + settings.fileTypeDesc + ')。';
                        break;
                }
            }
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == 1) {
                            var overwrite = ShowConfirm('同名称的文件 "' + file.name + '" 已经存在于服务器上。\n您想要替换现有文件？');
                            if (!overwrite) {
                                this.cancelUpload(file.id);
                                $('#' + file.id).remove();
        ...
        onUploadSuccess: function (file, data, response) {
            ...
            if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
                $('#' + file.id).find('.data').css("color", "#038813").html(' - 完成');
            }
